...
class Rocket(pygame.sprite.Sprite):
    ...
    def reset(self):
        ...
        # Position of rocket (starts in the middle)
        self.y = np.random.randint(350, 450)
        self.x = 0
...
class LunarLander:
    ...
    def __init__(self):
        ...
        self.reset()
        self.max_vals = np.array([140])
        self.observation_dim = len(self.max_vals)
        self.action_dim = 2
        self.prev_reward = None
        self.reward = None
        self.reward_function = """
        reward = 0 if self.rocket.yspeed < 0 else 140 / max(abs(self.rocket.yspeed), 10) / 14

        if self.game_over:
            if self.won:
                reward = 1000 + self.rocket.fuel * 1000
            else:
                reward = -100"""

    def get_state(self):
        return np.array([self.rocket.yspeed]) / self.max_vals
